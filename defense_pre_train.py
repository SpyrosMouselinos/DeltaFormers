...
def train_model(config, device, experiment_name='experiment_1', load_from=None, clvr_path='data/',
                ...
                freeze_exponential_growth=False, train_percentage=20):
    ...
    if load_from is not None:
        ...
        train_set = AVAILABLE_DATASETS[config['model_architecture']][1](config=config, split='Defense',
                                                                        ...
                                                                        questions_path=questions_path,
                                                                        scenes_path=scenes_path,
                                                                        use_cache=use_cache, return_program=True, effective_range_offset=0, randomize_range=False, effective_range=effective_range)
        ...
        val_set = AVAILABLE_DATASETS[config['model_architecture']][1](config=config, split='val',
                                                                      ...
                                                                      questions_path=questions_path,
                                                                      scenes_path=scenes_path, use_cache=use_cache,
                                                                      return_program=True, effective_range_offset=0, randomize_range=False, effective_range=16)
            ...
            if running_train_batch_index % config['validate_every'] == 0 and running_train_batch_index > 0:
                ...
                if total_val_acc / (val_batch_index + 1) > best_val_acc:
                    ...
                    save_all(model, optimizer, scheduler, bs_scheduler, epoch, best_val_acc,
                             f'./results/{experiment_name}', train_percentage)
                    if osp.exists(f'./results/{experiment_name}/model_at_{train_percentage}.pt'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_percentage', type=float, help='The percentage of Defense Dataset to be used as training', default=100)
    ...
    train_model(config=args.config, device=args.device, experiment_name=args.name, load_from=args.load_from,
                ...
                freeze_exponential_growth=args.freeze_exponential_growth, train_percentage=args.train_percentage)
