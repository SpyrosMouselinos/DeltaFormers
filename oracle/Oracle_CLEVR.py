...
import oracle.question_engine as qeng
...
class Oracle:
    ...
    @staticmethod
    def __clean_program__(question_program):
        program = []
        dirty_program = question_program
        for dirty_entry in dirty_program:
            if dirty_entry == {'PAD'}:
                continue
            #if dirty_entry['type'] == 'scene':
            if dirty_entry['function'] == 'scene':
                pass
            else:
                try:
    ...
    @staticmethod
    def __clean_scene__(scene):
        if 'relationships' in scene:
            scene.pop('relationships')
        scene['relationships'] = compute_all_relationships(scene)
        return scene

    def __single_pass__(self, question_program, scene, real_answer=None):
        if real_answer is not None:
            real_answer = str(real_answer).lower()
            if real_answer == 'yes' or real_answer == 'Yes':
                real_answer = 'true'
            elif real_answer == 'no' or real_answer == 'No':
                real_answer = 'false'
        question_program = self.__clean_program__(question_program)
        scene = self.__clean_scene__(scene)
        new_answer = instantiate_templates_dfs(scene_struct=scene, program=question_program, metadata=self.metadata)
        new_answer = str(new_answer).lower()
        if real_answer is not None:
            return new_answer, 1.0 * (real_answer == new_answer)
        else:
            return new_answer

    def __call__(self, question_program, scene, real_answer=None):
        if isinstance(question_program, list):
            if isinstance(scene, list):
                assert len(question_program) == len(scene)
                if isinstance(real_answer, list):
                    pass
                elif real_answer is None:
                    real_answer = [None] * len(scene)
                new_answers, equals = [], []
                for q, s, r in zip(question_program, scene, real_answer):
                    new_answer = self.__single_pass__(q, s, r)
                    new_answers.append(new_answer)
                    #equals.append(eq)
                return new_answers #equals
            elif isinstance(scene, dict):
                return self.__single_pass__(question_program, scene, real_answer)


# with open('../data/CLEVR_val_questions.json', 'r') as fin:
#     questions = json.load(fin)
#     questions = questions['questions']
#
# with open('../data/CLEVR_val_scenes.json', 'r') as fin:
#     scenes = json.load(fin)
#     scenes = scenes['scenes']
#
# oracle = Oracle(metadata_path='./metadata.json')
# oracle(question_program=questions[0]['program'], scene=scenes[0])
