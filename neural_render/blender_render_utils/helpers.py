...
from neural_render.blender_render_utils.constants import MAYBE_LIST_FLOAT, MAYBE_LIST_INT, \
    NESTED_MAYBE_LIST_FLOAT, \
    NESTED_MAYBE_LIST_INT, OUTPUT_IMAGE_DIR_, OUTPUT_SCENE_DIR_, OUTPUT_SCENE_FILE_, UP_TO_HERE_
from neural_render.blender_render_utils.constants import find_platform_exec
...
def serialize_dict(the_dict: dict):
    string = json.dumps(the_dict)
    return str(string)
...
def command_template(num_images,
    ...
    cmd_template = f'{find_platform_exec()}  -noaudio --background --python {UP_TO_HERE_}/generation/render_images.py > /dev/null 2>&1 -- --num_images={num_images} \
      ...
      --split={split} \
      --object_properties={serialize_dict(object_properties)}\
      --output_image_dir={output_image_dir} \
      --output_scene_dir={output_scene_dir} \
      --output_scene_file={output_scene_file} \
      --use_gpu=1 --render_num_samples=512 --width=480 --height=320 --start_idx={start_idx}'
    return cmd_template
