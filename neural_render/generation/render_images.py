...
def deserialize_dict(the_binary: str):
    return ast.literal_eval(the_binary)
...
def main(args):
    ...
    args.object_properties = deserialize_dict(args.object_properties)
    args.num_objects = []
...
def render_scene(args,
    ...
    # Load materials
    butils.load_materials(args.material_dir)
    ...
    if args.camera_jitter[current_item] > 0:
        for i in range(3):
            bpy.data.objects['Camera'].location[i] += args.camera_jitter[current_item]
    ...
    if args.key_light_jitter[current_item] > 0:
        for i in range(3):
            bpy.data.objects['Lamp_Key'].location[i] += args.key_light_jitter[current_item]
    if args.back_light_jitter[current_item] > 0:
        for i in range(3):
            bpy.data.objects['Lamp_Back'].location[i] += args.back_light_jitter[current_item]
    if args.fill_light_jitter[current_item] > 0:
        for i in range(3):
            bpy.data.objects['Lamp_Fill'].location[i] += args.fill_light_jitter[current_item]
...
def add_random_objects(current_item, scene_struct, args, camera, old_behaviour=False):
    ...
    for i in range(args.num_objects[current_item]):
        ...
        butils.add_object(args.shape_dir, obj_name, r, (x, y), theta=theta)
        ...
        butils.add_material(mat_name, Color=rgba)

        # Record data about the object in the scene data structure
        pixel_coords = butils.get_camera_coords(camera, obj.location)
        objects.append({
            'shape': obj_name_out,
            'size': size_name,
...
def render_shadeless(blender_objects, path='flat.png', old_behaviour=False):
    ...
    butils.set_layer(bpy.data.objects['Lamp_Key'], 2)
    butils.set_layer(bpy.data.objects['Lamp_Fill'], 2)
    butils.set_layer(bpy.data.objects['Lamp_Back'], 2)
    butils.set_layer(bpy.data.objects['Ground'], 2)
    ...
    butils.set_layer(bpy.data.objects['Lamp_Key'], 0)
    butils.set_layer(bpy.data.objects['Lamp_Fill'], 0)
    butils.set_layer(bpy.data.objects['Lamp_Back'], 0)
    butils.set_layer(bpy.data.objects['Ground'], 0)
